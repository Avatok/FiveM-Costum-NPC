...
local interactingWithPed = false

Citizen.CreateThread(function()
    while true do
        if isAtPed then
            showInfobar('Drücke ~g~E~s~, um den Befehl auszulösen')
            if IsControlJustReleased(0, 38) and not interactingWithPed then
                interactingWithPed = true
                ExecuteCommand('Befehl')  -- Befehl der ausgeführt wird
            end
        else
            interactingWithPed = false
        end
        Citizen.Wait(1)
    end
end)
